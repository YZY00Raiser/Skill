...
class Test{ClassName}(CustomTestCase):
    ...
    [Test Target] {TargetComponents}
    
    Adapted from GPU test: {GpuTest}
...
def generate_lora_test(feature_name: str, test_scenarios: list) -> str:
    ...
    for scenario in test_scenarios:
        ...
        content += CLASS_TEMPLATE.format(
            ...
            TargetComponents=scenario['target'],
            GpuTest=scenario.get('gpu_test', 'unknown'),
            Adaptations=format_adaptations(scenario.get('adaptations', [])),
            ModelPath="LLAMA_3_2_1B_INSTRUCT_WEIGHTS_PATH",
            ExtraArgs=extra_args,
            test_name=scenario['test_method'],
            TestDesc=scenario.get('test_desc', 'basic inference'),
...
def format_adaptations(adaptations: list) -> str:
    """Format adaptation notes for test file docstring (English only).
    """
    if not adaptations:
        return "- None"
...
LORA_TP_SCENARIO = {
    ...
    "category_cn": "张量并行",  # For README only
    "target": "LoRA inference under TP=2",
    "target_cn": "TP=2下的LoRA推理",  # For README only
...
LORA_RADIX_CACHE_SCENARIO = {
    ...
    "category": "Radix Cache",
    "category_cn": "Radix缓存",
    "target": "KV cache reuse with LoRA",
    "target_cn": "LoRA KV缓存复用",
...
EP_AUTO_SCENARIO = {
    ...
    "category": "Expert Parallelism",
    "category_cn": "专家并行",
    "target": "DeepEP auto mode inference",
    "target_cn": "DeepEP自动模式推理",
...
CP_ATTN_CP4_SCENARIO = {
    ...
    "category_cn": "上下文并行",
    "target": "Context parallelism with attention CP size = 4",
    "target_cn": "注意力CP大小为4的上下文并行",
